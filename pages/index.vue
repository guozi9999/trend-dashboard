...
<template>
  <div>
    <UContainer class="py-8">
      ...
      <div v-if="dashboard" class="space-y-6">
            ...
            <div class="mt-2 grid gap-2 sm:grid-cols-[minmax(180px,1fr)_86px_128px]">
              <div class="relative min-w-0">
                <UIcon
                  ...
                  class="pointer-events-none absolute left-3 top-5 z-10 size-5 -translate-y-1/2 text-zinc-400"
                />
                <UInput
                  id="stock-search"
                  v-model="searchText"
              ...
              <UButton
                ...
                class="h-10 justify-center whitespace-nowrap"
              >
                查询
              </UButton>
              <UButton
        ...
        <section class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <UBadge :color="signalTone" size="lg" variant="soft">
              {{ dashboard.dividend.signal.label }}
            </UBadge>
          </div>

          <p class="mt-4 max-w-4xl text-sm leading-6 text-zinc-600">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <div class="metric-tile rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-zinc-500">最新价</p>
            <p class="mt-3 text-3xl font-semibold text-zinc-950">{{ formatMoney(dashboard.stock.closePrice) }}</p>
            <p class="mt-2 text-xs text-zinc-500">市值 {{ formatMarketCap(dashboard.stock.totalMarketCap) }}</p>
          </div>

          <div class="metric-tile rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-zinc-500">最新年度股息率</p>
            <p class="mt-3 text-3xl font-semibold text-emerald-700">{{ formatPercent(latestYield) }}</p>
            <p class="mt-2 text-xs text-zinc-500">
          ...
          <div class="metric-tile rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-zinc-500">{{ dashboard.dividend.forecastFiscalYear || '-' }}E 预测股息率</p>
            <p class="mt-3 text-3xl font-semibold text-emerald-700">{{ formatPercent(dashboard.dividend.forecastDividendYield) }}</p>
            <p class="mt-2 text-xs text-zinc-500">
          ...
          <div class="metric-tile rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-zinc-500">中国 10Y 国债</p>
            <p class="mt-3 text-3xl font-semibold text-zinc-950">{{ formatPercent(cn10y) }}</p>
            <p class="mt-2 text-xs text-zinc-500">风险补偿线 {{ formatPercent(dashboard.dividend.riskPremiumTargetYield) }}</p>
          </div>
          ...
          <div class="metric-tile rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-zinc-500">连续分红年数</p>
            <p class="mt-3 text-3xl font-semibold text-zinc-950">{{ dashboard.dividend.continuousDividendYears }}</p>
            <p class="mt-2 text-xs text-zinc-500">近 5 年 CAGR {{ formatPercent(dashboard.dividend.fiveYearCashCagr) }}</p>
          </div>
        </section>
        ...
        <section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 class="text-lg font-semibold text-zinc-950">年度现金分红</h3>
                <p class="mt-1 text-sm text-zinc-500">按报告期归集，含年报、中报、三季报等现金分红。</p>
              </div>
              <UBadge color="gray" variant="soft">近 {{ annualRows.length }} 年</UBadge>
            </div>
            ...
            <div class="mt-5 space-y-3">
              <div
                ...
                :key="item.fiscalYear"
                class="grid gap-3 sm:grid-cols-[72px_minmax(0,1fr)_96px_96px]"
              >
                <span class="text-sm font-medium text-zinc-700">{{ item.fiscalYear }}</span>
                <div class="h-7 overflow-hidden rounded-md bg-zinc-100">
                  <div
                    class="h-full rounded-md bg-emerald-500"
                    :style="{ width: annualBarWidth(item.cashPerShare) }"
                  />
                </div>
                <span class="text-sm text-zinc-700">{{ formatNumber(item.cashPerShare, 4) }} 元/股</span>
                <span class="text-sm text-zinc-500">{{ formatPercent(item.yieldAtCurrentPrice) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              <div class="mt-4 space-y-3">
                ...
                <div class="flex items-center justify-between gap-4">
                  <span class="text-sm text-zinc-500">达到 4.5% 股息率</span>
                  <span class="text-lg font-semibold text-zinc-950">{{ formatMoney(dashboard.dividend.priceForFourPointFivePercentYield) }}</span>
                </div>
                <div class="flex items-center justify-between gap-4">
                  <span class="text-sm text-zinc-500">近 5 年平均股息率</span>
                  <span class="text-lg font-semibold text-zinc-950">{{ formatPercent(dashboard.dividend.fiveYearAverageYield) }}</span>
                </div>
              </div>
            </div>

            <div class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
              <h3 class="text-lg font-semibold text-zinc-950">国债对比</h3>
              <div class="mt-4 divide-y divide-zinc-100">
                <div class="flex items-center justify-between py-2">
                  <span class="text-sm text-zinc-500">中国 2Y</span>
                  <span class="font-medium text-zinc-900">{{ formatPercent(dashboard.treasury.latest.cn2y) }}</span>
                </div>
                <div class="flex items-center justify-between py-2">
                  <span class="text-sm text-zinc-500">中国 5Y</span>
                  <span class="font-medium text-zinc-900">{{ formatPercent(dashboard.treasury.latest.cn5y) }}</span>
                </div>
                <div class="flex items-center justify-between py-2">
                  <span class="text-sm text-zinc-500">中国 10Y</span>
                  <span class="font-medium text-zinc-900">{{ formatPercent(dashboard.treasury.latest.cn10y) }}</span>
                </div>
                <div class="flex items-center justify-between py-2">
                  <span class="text-sm text-zinc-500">中国 30Y</span>
        ...
        <section class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 class="text-lg font-semibold text-zinc-950">年度股价与股息率区间</h3>
              <p class="mt-1 text-sm text-zinc-500">按文章原文的股息率表思路：用年度分红除以当年最高价/最低价，得到每年的最低/最高股息率。</p>
            </div>
            <UBadge color="gray" variant="soft">{{ annualRows.length }} 年</UBadge>
          </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-left text-sm">
              ...
              <tbody class="divide-y divide-zinc-100">
                <tr v-for="item in annualRows" :key="`range-${item.fiscalYear}`" class="hover:bg-zinc-50">
                  <td class="whitespace-nowrap px-3 py-3 font-medium text-zinc-900">{{ item.fiscalYear }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatNumber(item.cashPerShare, 4) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatMoney(item.lowestPrice) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(item.lowestPriceDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 font-medium text-emerald-700">{{ formatPercent(item.highestDividendYield) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatMoney(item.highestPrice) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(item.highestPriceDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatPercent(item.lowestDividendYield) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <section class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 class="text-lg font-semibold text-zinc-950">全部分红记录</h3>
              <p class="mt-1 text-sm text-zinc-500">展示现金分红方案，以及公告日、股权登记日、除权除息日和派息日。</p>
            </div>
            <UBadge color="gray" variant="soft">{{ dividendRows.length }} 条</UBadge>
          </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-left text-sm">
              ...
              <tbody class="divide-y divide-zinc-100">
                <tr v-for="row in dividendRows" :key="`${row.reportDate}-${row.planProfile}`" class="hover:bg-zinc-50">
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatDate(row.reportDate) }}</td>
                  <td class="min-w-48 px-3 py-3 text-zinc-900">{{ row.planProfile || '-' }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatNumber(row.pretaxBonusRmbPer10, 3) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatNumber(row.cashPerShare, 4) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ formatPercent(row.dividendYieldAtAnnouncement) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(row.noticeDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(row.equityRecordDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(row.exDividendDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-500">{{ formatDate(row.payCashDate) }}</td>
                  <td class="whitespace-nowrap px-3 py-3 text-zinc-700">{{ row.assignProgress || '-' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <section class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
          <div class="grid gap-4 md:grid-cols-2">
            <div>
              <h3 class="text-lg font-semibold text-zinc-950">方法阈值</h3>
              <div class="mt-4 grid gap-3">
                <div class="rounded-md border border-emerald-100 bg-emerald-50 p-3">
                  <p class="font-medium text-emerald-900">4.5%+ 且息差 250BP+</p>
                  <p class="mt-1 text-sm text-emerald-800">深度股息率区间。</p>
                </div>
                <div class="rounded-md border border-blue-100 bg-blue-50 p-3">
                  <p class="font-medium text-blue-900">4%+ 且息差 200BP+</p>
                  <p class="mt-1 text-sm text-blue-800">进入攒股观察区间。</p>
                </div>
                <div class="rounded-md border border-amber-100 bg-amber-50 p-3">
                  <p class="font-medium text-amber-900">高于 10Y 国债 100BP+</p>
                  <p class="mt-1 text-sm text-amber-800">具备基础风险补偿。</p>
                </div>
              </div>
            </div>

            <div>
              <h3 class="text-lg font-semibold text-zinc-950">计算说明</h3>
        ...
        <section class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
          <div class="grid gap-4 md:grid-cols-2">
            <div>
              <h3 class="text-lg font-semibold text-zinc-950">数据来源</h3>
            ...
            <div>
              <h3 class="text-lg font-semibold text-zinc-950">分红日期口径</h3>
              <div class="mt-4 space-y-3 text-sm leading-6 text-zinc-600">
                <p>公告日：公司披露分红方案的日期。</p>
      ...
      <div v-else-if="pending" class="grid gap-4 md:grid-cols-3">
        <USkeleton v-for="item in 6" :key="item" class="h-32 rounded-md" />
      </div>
    </UContainer>
  </div>
</template>
